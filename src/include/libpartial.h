...
#define ASSERT(x, m) if(!(x)) { fflush(stdout); fprintf(stderr, "error: %s\n", m); perror("error"); fflush(stderr); exit(1); }
	...
	STATIC_INLINE void hexToBytes(const char *hex, uint8_t ** buffer,
				      size_t * bytes) {
		size_t i;
		*bytes = strlen(hex) / 2;
	...
	STATIC_INLINE void hexToInts(const char *hex, unsigned int **buffer,
				     size_t * bytes) {
		size_t i;
		*bytes = strlen(hex) / 2;
	...
	typedef int (*readFunc) (struct io_func_struct * io, off_t location,
				 size_t size, void *buffer);
	typedef int (*writeFunc) (struct io_func_struct * io, off_t location,
				  size_t size, void *buffer);
...
#pragma pack(pop)
	...
	typedef void (*PartialZipProgressCallback) (ZipInfo * info,
						    CDFile * file,
						    size_t progress);
	...
	int download_file_from_zip(const char *url, const char *path,
				   const char *output,
				   PartialZipProgressCallback progressCallback);
	...
	void PartialZipSetProgressCallback(ZipInfo * info,
